...
function  makeCourse(data,pic) {
	...
	var heigt_=0,width_=0;
	for (var i = 0; i < data.length; i++) {
		for(var a=0;a<data[i][1].length;a++){
			heigt_+=data[i][0];
			break;
		}
	}
	...
	img.onload = function(){
		...
		for (var i = 0; i < data.length; i++) {
			for(var a=0;a<data[i][1].length;a++){
				var tt=0;
				if(data[i][1][a][1])tt=data[i][1][a][1].length;
				if(maxWidth[a]<data[i][1][a][0])maxWidth[a]=data[i][1][a][0];
				for(var b=0;b<tt;b++){
					if(data[i][1][a][1][0].length>1){
						canvas.font="normal normal 700 14px Microsoft Yahei";
						var temp=canvas.measureText(data[i][1][a][1][b][0]).width+30;
						if(maxWidth[a]<temp)maxWidth[a]=temp;
						canvas.font="normal normal 400 14px Microsoft Yahei";
						temp=canvas.measureText(data[i][1][a][1][b][1]).width+20+canvas.measureText(data[i][1][a][1][b][2]).width+30;
						if(maxWidth[a]<temp)maxWidth[a]=temp;
					}
				}
			}
		...
		for (var i = 0; i < data.length; i++) {
			...
			canvas.fillRect(0,yTable,canvasE.clientWidth,data[i][0]);
			yTable+=data[i][0];
			canvas.beginPath();
			...
			for(var a=0;a<data[i][1].length;a++){
				...
				for(var b=0;b<tt;b++){
					...
					if(data[i][1][a][1][0].length>1){
						canvas.font="normal normal 700 14px Microsoft Yahei";
						canvas.fillText(data[i][1][a][1][b][0],xTable+5,y_);
						canvas.font="normal normal 400 14px Microsoft Yahei";
						canvas.fillText(data[i][1][a][1][b][1],xTable+5,y_+16);
						canvas.fillText(data[i][1][a][1][b][2],xTable+5+canvas.measureText(data[i][1][a][1][b][1]).width+20,y_+16);
						canvas.fillText(data[i][1][a][1][b][3],xTable+5,y_+16+16);
						y_+=16*3+5;
					}
					else{
						if (data[i][1][a][2]=="center") {
							canvas.fillText(data[i][1][a][1],xTable+5,y_+(data[i][0]-14)/2-5);
						}
						else if(data[i][1][a][2]=="vertical"){
							for(var c=0;c<data[i][1][a][1].length;c++){
								canvas.fillText(data[i][1][a][1].substring(c,c+1),xTable+12,y_+5+c*22);
							}
						}
						else{canvas.fillText(data[i][1][a][1],xTable+5,y_);}
						break;
					}
				}
				...
				xTable+=maxWidth[a];
				canvas.beginPath();
				canvas.moveTo(xTable,yTable-data[i][0]);
